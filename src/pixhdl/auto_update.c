...
int update (const char * current_version)
{
    ...
    check(strlen(latest_version),
          "Couldn't get latest version. Check your internet connection.");
    ...
    if (strcmp(latest_version, current_version) == 0) {
        ...
        log_info("Your version of pixhdl (v%s) is ahead of the "\
                 "latest stable release (v%s).",
        ...
        log_info("Do you want to install v%s of pixhdl? "\
                 "(Current version: v%s)",
        ...
        do {
            ...
            if (choice == 'y' || choice == 'Y') {
                ...
                log_info("Downloading and installing version v%s of pixhdl...",
                         latest_version);
                // Launch the update command
                ret = system(UPDATE_COMMAND);
                // Check that the update command was successful
                check(ret == 0, "Error downloading version v%s.",
                                latest_version);
                // Inform the user that the installation is complete
                log_info("Done! Version v%s of pixhdl is now installed.",
                         latest_version);
            // If the choice is no
            ...
            // If the choice is something else
                ...
                log_info("Please enter y/Y (yes) to continue "\
                         "with the update, or n/N (no) to abort it.");
            }

            // Flush input to get rid of eventual characters that are still on
            // the buffer
