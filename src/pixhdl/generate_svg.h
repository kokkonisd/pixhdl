...
#define SVG_HEADER "<svg version='1.1' baseProfile='full' width='%.2f' "\
                   "height='%.2f' xmlns='http://www.w3.org/2000/svg'>\n"

/** The Entity rectangle. */
#define ENTITY_RECTANGLE "<rect width='%.2f' height='%.2f' x='%.2f' y='%.2f' "\
                         "fill='none' stroke='black' stroke-width='%d' />\n"
/** The Entity name text. */
#define ENTITY_NAME "<text text-anchor='middle' x='%.2f' y='%.2f' "\
                    "font-size='%d' fill='black' font-family='Courier'>"\
                    "%s</text>\n"

/** The arrow head path. */
#define ARROW_HEAD "<path d='M%.2f %.2f L%.2f %.2f L%.2f %.2f "\
                   "L%.2f %.2f Z'/>\n"
/** A simple line connecting two points. */
#define SIMPLE_LINE "<line x1='%.2f' y1='%.2f' x2='%.2f' y2='%.2f' "\
                    "stroke='black' stroke-width='5' fill='transparent'/>\n"

/**
 * A small triangle (to be placed in the bottom left corner of the entity) that
 * indicates that the entity is -- at least partially -- synchronous, meaning
 * that it has a CLOCK input.
 */
#define CLOCK_TRIANGLE "<path d='M%.2f %.2f L%.2f %.2f L%.2f %.2f Z' "\
                       "stroke='black' stroke-width='3' fill='transparent'/>\n"

/** The text for the generic input names. */
#define GENERIC_SIGNAL_NAME "<text text-anchor='begin' x='%.2f' y='%.2f' "\
                            "font-size='%d' fill='black' "\
                            "font-family='Courier'>%s : %s</text>\n"
/** The text for the IN signal names. */
#define IN_SIGNAL_NAME "<text text-anchor='end' x='%.2f' y='%.2f' "\
                       "font-size='%d' fill='black' font-family='Courier'>"\
                       "%s</text>\n"
/** The text for the OUT signal names. */
#define OUT_SIGNAL_NAME "<text text-anchor='begin' x='%.2f' y='%.2f' "\
                        "font-size='%d' fill='black' font-family='Courier'>"\
                        "%s</text>\n"
/** The text for the INOUT signal names. */
#define INOUT_SIGNAL_NAME "<text text-anchor='end' x='%.2f' y='%.2f' "\
                          "font-size='%d' fill='black' font-family='Courier' "\
                          "transform='rotate(270 %.2f,%.2f)'>%s</text>\n"

/** The text for the horizontal input/output lengths. */
#define HORIZONTAL_LENGTH_TEXT "<text text-anchor='middle' x='%.2f' y='%.2f' "\
                               "font-size='%d' fill='black' "\
                               "font-family='Courier'>%s</text>\n"
/** The text for the vertical throughput lengths. */
#define VERTICAL_LENGTH_TEXT "<text text-anchor='middle' x='%.2f' y='%.2f' "\
                             "font-size='%d' fill='black' "\
                             "font-family='Courier' "\
                             "transform='rotate(270 %.2f,%.2f)'>%s</text>\n"
/** The small line (slash) marking the length of a signal. */
#define LENGTH_SLASH "<line x1='%.2f' y1='%.2f' x2='%.2f' y2='%.2f' "\
                     "stroke='black' stroke-width='3' fill='transparent' />\n"
...
/**
 * Enumeration of the possible directions of an arrow line/arrow head (`LEFT`,
 * `RIGHT`, `UP`, `DOWN`).
 */
typedef enum {
    RIGHT,
 ...
 *
 * @param fo  (`FILE *`): Output SVG file for the arrow head to be written
 *                        into.
 * @param x    (`float`): The `x` coordinate of the point.
 * @param y    (`float`): The `y` coordinate of the point
 * @param dir (`arrDir`): The direction of the arrow head (`LEFT`, `RIGHT`,
 *                        `UP`, `DOWN`).
 */
void drawArrowHead (FILE * fo, float x, float y, arrDir dir);

/**
 * Draws a line with a simple arrow at its end in an SVG file (fo). Only
 * handles purely horizontal/vertical lines.
 *
 ...
 * @param dir     (`arrDir`): The direction of the arrow line (`LEFT`, `RIGHT`,
 *                            `UP`, `DOWN`).
 */
 ...
 *
 * @param  ent      (`Entity *`): The entity object from which to generate an
 *                                image.
