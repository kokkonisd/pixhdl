...
void drawSimpleArrowLine (FILE * fo, float start_x, float start_y, float end_x,
                          float end_y, arrDir dir)
{
    // Draw the arrow going in the entity rect
    switch (dir) {
        case LEFT:
            fprintf(fo, SIMPLE_LINE, start_x + ARROW_WIDTH,
                    start_y, end_x, end_y);
            drawArrowHead(fo, start_x + ARROW_WIDTH, start_y, dir);
            break;

        case UP:
            fprintf(fo, SIMPLE_LINE, start_x,
                    start_y, end_x, end_y + ARROW_WIDTH);
            drawArrowHead(fo, end_x, end_y + ARROW_WIDTH, dir);
            break;

        case DOWN:
            fprintf(fo, SIMPLE_LINE, start_x,
                    start_y - ARROW_WIDTH, end_x, end_y);
        ...
        case RIGHT:
            fprintf(fo, SIMPLE_LINE, start_x,
                    start_y, end_x - ARROW_WIDTH, end_y);
...
void drawDoubleArrowLine (FILE * fo, float start_x, float start_y, float end_x,
                          float end_y, arrDir dir)
{
    switch (dir) {
        case UP:
        case DOWN:
            fprintf(fo, SIMPLE_LINE, start_x,
                    start_y - ARROW_WIDTH, end_x, end_y + ARROW_WIDTH);
...
int generateSvgFromEntity (Entity * ent, char * filename)
{
    ...
    // Each time we need to account for the maximum port name length times the
    // approximate character width (font_size * 0.55), adding 3 extra
    // characters for spacing. To that we also need to add the arrow length.
    arrow_length_left = max(ARROW_LENGTH,
                            (ent->max_length_size_in + 2) * LENGTH_FONT_SIZE)
                        + ARROW_WIDTH * 1.5;
    arrow_length_right = max(ARROW_LENGTH,
                             (ent->max_length_size_out + 2) * LENGTH_FONT_SIZE)
                         + ARROW_WIDTH * 1.5;
    port_width_left = arrow_length_left + (ent->max_name_size_in + 3) *
                      PORT_NAME_FONT_SIZE * 0.55;
    port_width_right = arrow_length_right + (ent->max_name_size_out + 3) *
                       PORT_NAME_FONT_SIZE * 0.55;
    ...
    //     1. The entity's name width (in pixels, so as before that is
    //        font_size * 0.55) plus 2 characters for spacing
    //     2. The width produced by the INOUT channels, if they exist
    rect_width = max(ENTITY_NAME_FONT_SIZE * (strlen(ent->name) + 2),
                     port_height * ent->count_inout);
    if (ent->count_generics > 0)
        rect_width += (ent->max_name_size_generics +
                       ent->max_length_size_generics + 2)
                      * PORT_NAME_FONT_SIZE * 0.55;
    // The height of the rectangle is also the maximum between two floats:
    //     1. The maximum between INs and OUTs times the width of one arrow
    //        head
    //     2. The entity's name font size (which basically means "enough place
    //        for the entity's name")
    rect_height = max(max(ent->count_in, ent->count_out) * port_height,
    ...
    // The total SVG width is then simply the sum of the two port widths plus
    // the entity rect width
    svg_width = rect_width + RECTANGLE_STROKE_WIDTH * 2 +
                port_width_left + port_width_right;

    // The total SVG height is at least equal to the entity rect height plus
    // its stroke width
    svg_height = rect_height + RECTANGLE_STROKE_WIDTH;

    // If there are INOUT signals, we need to add their height too
    arrow_length_bottom = max(ARROW_LENGTH,
                              (ent->max_length_size_inout + 2) *
                              LENGTH_FONT_SIZE)
                          + ARROW_WIDTH;

    if (ent->count_inout > 0)
        svg_height += arrow_length_bottom +
                      (PORT_NAME_FONT_SIZE * 0.55 *
                       (ent->max_name_size_inout + 2));

    // Open the output file, we're ready to produce the image
    svg_file = fopen(filename, "w");

    // First off, print the SVG header along with the width and height of the
    // image
    ...
    for (i = 0; i < ent->count_generics; i++) {
        fprintf(svg_file, GENERIC_SIGNAL_NAME, port_width_left +
                                                   RECTANGLE_STROKE_WIDTH +
                                                   PORT_NAME_FONT_SIZE,
    ...
    // Draw the IN signals
    //
    // We need to establish a step (aka how much space is vertically allocated
    // to each signal) as well as a cur_pos, which will tell us where to draw
    // the next signal vertically. The step is simply the rectangle's height
    // divided by the number of IN signals, while the initial Y position is
    // equal to half a step plus an offset due to the stroke width of the
    // entity rect.
    ...
    for (i = 0; i < ent->count_in; i++) {
        ...
        fprintf(svg_file, IN_SIGNAL_NAME, port_width_left -
                                              arrow_length_left -
                                              PORT_NAME_FONT_SIZE * 0.55,
        ...
        fprintf(svg_file, HORIZONTAL_LENGTH_TEXT, port_width_left -
                                                      arrow_length_left / 2,
                                                  cur_pos - 15,
                                                  LENGTH_FONT_SIZE,
    ...
    // to each signal) as well as a cur_pos, which will tell us where to draw
    // the next signal vertically. The step is simply the rectangle's height
    // divided by the number of OUT signals, while the initial Y position is
    // equal to half a step plus an offset due to the stroke width of the
    // entity rect.
    ...
    for (i = 0; i < ent->count_out; i++) {
        ...
        // Draw a slash to indicate signal length in bits
        fprintf(svg_file, LENGTH_SLASH, port_width_left + rect_width +
                                            arrow_length_right / 2 - 10,
                                        cur_pos + 10,
                                        port_width_left + rect_width +
                                            arrow_length_right / 2 + 10,
                                        cur_pos - 10);
        ...
        // Draw the signal's name
        fprintf(svg_file, OUT_SIGNAL_NAME, port_width_left + rect_width +
                                               arrow_length_right +
                                               PORT_NAME_FONT_SIZE * 0.55,
                                           cur_pos +
                                               PORT_NAME_FONT_SIZE * 0.25,
                                           PORT_NAME_FONT_SIZE,
    ...
    //
    // We need to establish a step (aka how much space is horizontally
    // allocated to each signal) as well as a cur_pos, which will tell us where
    // to draw the next signal horizontally. The step is simply the rectangle's
    // width divided by the number of INOUT signals, while the initial X
    // position is equal to half a step plus an offset due to the stroke width
    // of the entity rect.
    ...
    for (i = 0; i < ent->count_inout; i++) {
        ...
        fprintf(svg_file, LENGTH_SLASH, cur_pos - 10,
                                        rect_height + RECTANGLE_STROKE_WIDTH +
                                            arrow_length_bottom / 2 - 10,
                                        cur_pos + 10,
                                        rect_height + RECTANGLE_STROKE_WIDTH +
                                            arrow_length_bottom / 2 + 10);

        // Draw the signal's name
        fprintf(svg_file, INOUT_SIGNAL_NAME, cur_pos +
                                                 PORT_NAME_FONT_SIZE * 0.25,
                                             svg_height -
                                                 (ent->max_name_size_inout + 1)
                                                 * PORT_NAME_FONT_SIZE * 0.55,
                                             PORT_NAME_FONT_SIZE,
                                             cur_pos +
                                                 PORT_NAME_FONT_SIZE * 0.25,
